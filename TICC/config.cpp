...
void getLine()
{
  ...
  for ( inputLineIndex = 0; ; )
  {
    ...
    {
      inputLine[inputLineIndex++] = newChar;
    }
  }
}
...
void getInt64(int64_t *result, int source)
{
  ...
  if (newChar == '+')
  {
    newChar = getChar(source);    // allow leading plus
  }
  ...
  if ( (newChar == 'e') || (newChar == 'E') )
  {
    ...
    while (newChar == '0')
    {
      newChar = getChar(source);   // bypass exponent leading zeros
    }
    while(isdigit(newChar))
    {
      exponentInteger *= 10;
...
{
  int valid = 1;
  Serial.println("Set mode.  Valid single-letter commands are:")
  Serial.println();
...
void pollChar(struct config_t *pConfigInfo)
{
  ...
  {
    c = 0x00;
  }; // so don't use space as the poll character!
...
void clockSpeed(struct config_t *pConfigInfo)
{
  ...
  if (( clock >= 1) && (clock <= 16) )
  {
    pConfigInfo->CLOCK_HZ = clock * 1000000 + mantissaFractionPart * 1000000 / mantissaFractionPartPower;
  }
}
...
void calibrationPeriods(struct config_t *pConfigInfo)
{
  ...
  if (getInt64new)
  {
    ...
    {
      pConfigInfo->CAL_PERIODS = periods;
    }
  }
}
...
void timeout(struct config_t *pConfigInfo)
{
  ...
  inputLineReadIndex = 0;
  getInt64(&timeout, 1);
  if ( (getInt64new) && (timeout >= 0) && (timeout <= 255) )
  {
    pConfigInfo->TIMEOUT = timeout;
  }
}
...
void ts_wrap(struct config_t *pConfigInfo)
{
  ...
  int64_t wrap;
  Serial.print("Timestamp Wraparound");
  Serial.print(pConfigInfo->WRAP), Serial.println(" Default is no wrap");
...
{
  Serial.print("Master or Client ")
  Serial.print(pConfigInfo->SYNC_MODE), Serial.println("   Choose M or C.  Default is M");
  char c = toupper(getChar());
  if ( (c == 'M') || (c == 'C') )
  {
...
{
  char delim[] = " ";
  Serial.print("Channel Names:");
  Serial.print(pConfigInfo->NAME[0]);
  Serial.print("/");
  Serial.println(pConfigInfo->NAME[1]);
...
{
  char delim[] = " ";
  Serial.print("Propagation Delay (ps):");
  Serial.print(int32_t(pConfigInfo->PROP_DELAY[0]));
  Serial.print("/");
  Serial.println(int32_t(pConfigInfo->PROP_DELAY[1]));
...
void doSetupMenu(struct config_t *pConfigInfo)      // also display the default values ----------------
{
  ...
  for ( ; ; )
  {
    ...
    Serial.print("C   clock speed in MHz (default 10)             ");
    printHzAsMHz(pConfigInfo->CLOCK_HZ), Serial.println();       // int_64
    Serial.print("D   coarse clock rate in us) (default 100)      ");
    printHzAsMHz(pConfigInfo->PICTICK_PS), Serial.println();   // int_64
    Serial.print("E   calibration periods (default 20)            ");
    Serial.println((int32_t)pConfigInfo->CAL_PERIODS);  // int_16, choices are 2, 10, 20, 40
    Serial.print("F   timeout (default 0x05)                      ");       // int16

    char str[8];sprintf(str, "0x%02X", (int32_t)pConfigInfo->TIMEOUT);Serial.println(str);

    Serial.print("G   ts wrap (default 0 = none)                  ");
    Serial.print(pConfigInfo->WRAP); Serial.println(); // int16_t
    Serial.print("H   sync:  master / client (default M)          ");
    Serial.print(pConfigInfo->SYNC_MODE); Serial.println();  // M (default) or S

    Serial.print("I   channel name (default A/B)                  ");
    Serial.print(pConfigInfo->NAME[0]);Serial.print('/');
    Serial.println(pConfigInfo->NAME[1]);

    Serial.print("J   prop delay (default 0)                      ");       // int_64, default 0
    Serial.print((int32_t)pConfigInfo->PROP_DELAY[0]);Serial.print(' ');
    Serial.println((int32_t)pConfigInfo->PROP_DELAY[1]);

    Serial.print("K   trigger edge (default R R)                  ");     // R(ising) or F(alling)
    Serial.print(pConfigInfo->START_EDGE[0]);Serial.print(' ');
    Serial.println(pConfigInfo->START_EDGE[1]);

    Serial.print("L   time dilation (default 2500)                ");       // int_64, default 2500
    Serial.print((int32_t)pConfigInfo->TIME_DILATION[0]);Serial.print(' ');
    Serial.println((int32_t)pConfigInfo->TIME_DILATION[1]);

    Serial.print("M   fixed time2 (default 0)                     ");   // int_64, default 0
    Serial.print((int32_t)pConfigInfo->FIXED_TIME2[0]);Serial.print(' ');
    Serial.println((int32_t)pConfigInfo->FIXED_TIME2[1]);

    Serial.print("N   fudge0 (default 0)                          ");   // int_64, default 0
    Serial.print((int32_t)pConfigInfo->FUDGE0[0]);Serial.print(' ');
...
void UserConfig(struct config_t *pConfigInfo)
{
  ...
  {
    c = Serial.read();   // eat any characters entered before we start  doSetupMenu()
  }

  if (configRequested)
  {
    doSetupMenu(pConfigInfo);
  }
}
